...
describe("Test refresh auth controller", () => {
  ...
  it("Test 2) refresh auth roken", async () => {
    ...
    const responseRefresh = await basicRouteAuth({
      userCredentials: userCredentialsRefresh,
      route: "auth/refresh-token",
    });

    let oldRefreshToken = await RefreshToken.findOne({
      token: responseLogin.body.token.refreshToken.token,
      email: adminOne.email,
    });

    const checkTokenData = await checkRefreshToken({
      tokenData: responseRefresh.body,
      userId: adminOne._id.toString(),
    });
